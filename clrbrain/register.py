...
def volumes_by_id(labels_img, labels_ref_lookup, resolution, level=None, 
    ...
    for key in ids:
        ...
        for label_id in label_ids:
            ...
            name = label[ontology.NODE][config.ABAKeys.NAME.value]
            if level is None or label_level == level:
                ...
                region_dict = {
                    config.ABAKeys.NAME.value: label[ontology.NODE][config.ABAKeys.NAME.value],
                    config.VOL_KEY: vol,
        ...
        if label_level == level:
            ...
            for parent in parents[::-1]:
                parent_level = labels_ref_lookup[
                    parent][ontology.NODE][config.ABAKeys.LEVEL.value]
            ...
            if label_level == level:
                ...
                if parents is not None:
                    ...
                    for parent in parents[::-1]:
                        parent_level = labels_ref_lookup[
                            abs(parent)][
                                ontology.NODE][config.ABAKeys.LEVEL.value]
                        if parent_level < level:
                            break
