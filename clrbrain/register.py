...
def register_volumes_mp(img_paths, labels_path, level, densities=False):
    start_time = time()
    '''
    ...
    for img_path in img_paths:
        pool_results.append(pool.apply_async(
            register_volumes, 
            args=(img_path, labels_path, level, densities)))
    vols = []
    paths = []
...
if __name__ == "__main__":
    ...
    if config.register_type is None:
        ...
        vol_dicts, json_paths = register_volumes_mp(
            cli.filenames, config.load_labels, config.labels_level, densities)
