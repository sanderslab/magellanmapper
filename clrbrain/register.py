...
def make_edge_images(path_atlas, show=True):
    ...
    dist_sitk = _get_edge_dist_groups(
        path_atlas, atlas_edge, labels_edge, labels_sitk, IMG_LABELS_DIST)
...
def merge_atlas_segmentations(path_fixed, show=True):
    ...
    dist_sitk = _get_edge_dist_groups(
        path_fixed, atlas_edge, labels_edge, labels_sitk_seg, 
        IMG_LABELS_SEG_DIST)
...
def _get_edge_dist_groups(path, atlas_edge, labels_edge, img_sitk, dist_suffix):
...
def measure_edge_dist(atlas_edge, labels_edge):
    ...
    dists = {}
    label_ids = np.unique(labels_edge)
    dists_wt = []
    tot_pxs = 0
    for label_id in label_ids:
        ...
        label_mask = labels_edge == label_id
