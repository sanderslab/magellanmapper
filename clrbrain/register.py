...
def make_edge_images(path_atlas, show=True):
    ...
    name = lib_clrbrain.get_filename_without_ext(path_atlas)
    _, dist_to_orig, dists = measure_edge_dist(
        atlas_edge, labels_edge, labels_img_np, name, "Manual", config.suffix)
    ...
    output_path = lib_clrbrain.combine_paths(
        path_atlas, IMG_LABELS_DIST, ext="csv") 
    df = stats.dict_to_data_frame(dists, output_path)
...
def merge_atlas_segmentations(path_fixed, show=True):
    ...
    print("\nMeasuring edge distance:")
    name = lib_clrbrain.get_filename_without_ext(path_fixed)
    _, dist_to_orig, dists = measure_edge_dist(
        atlas_edge, labels_edge, labels_img_np, name, "Watershed", 
        config.suffix)
    ...
    # write images and data frame
    write_reg_images(imgs_write, path_fixed)
    output_path = lib_clrbrain.combine_paths(
        path_fixed, IMG_LABELS_SEG_DIST, ext="csv") 
    df = stats.dict_to_data_frame(dists, output_path)

def measure_edge_dist(atlas_edge, labels_edge, labels_img_np=None, sample=None, 
                      seg_type=None, condition=None):
    ...
    Args:
        ...
        sample: Sample identification; defaults to None.
        seg_type: Segmentation type identifier; defaults to None.
        condition: Condition identifier; defaults to None.
    ...
    if labels_img_np is not None:
        ...
        for label_id in label_ids:
            ...
            dists.setdefault("Sample", []).append(sample)
            dists.setdefault("Seg", []).append(seg_type)
            dists.setdefault("Condition", []).append(condition)
