...
def volumes_by_id_compare(img_paths, labels_ref_lookup, unit_factor=None,
    ...
    Returns:
        Pandas data frame with volume-related metrics for each 
        sample.
    """
    start_time = time()
    ...
    df_out = vols.measure_labels_overlap(
        sample, labels_imgs, heat_map, spacing, unit_factor, 
        combine_sides and max_level is None, label_ids, grouping, df)
    
    # output volume stats CSV to atlas directory and append for 
    # combined CSVs
    if max_level is None:
        df_out = stats.data_frames_to_csv(df_out, df_path, sort_cols=sort_cols)
    elif df_level_path is not None:
        df_out = stats.data_frames_to_csv(
            df_out, df_level_path, sort_cols=sort_cols)
...
def main():
    ...
    if config.register_type is None:
        ...
        # compare volumes from two different atlases
