...
#'
#' @param vals List of values to compare.
#' @param conditions List of conditions corresponding to `vals`. Assumed to
#'   have at least two conditions except when `model` is "ttest" or "wilcoxon"
#'   (one-sample) or "shapiro". Only the first two sorted conditions will be
#'   used, ignoring all other conditions. For paired tests, the number of 
...
#' @param model One of [kModels] to apply, which should be a mean test such as 
#'   "ttest" or "wilcoxon".
#' @param paired True for paired test; defaults to FALSE.
#' @param reverse True to reverse the order of sorted conditions; defaults 
#'   to FALSE.
#' @return Data frame of with p-value and mean of differences columns.
meansModel <- function(vals, conditions, model, paired=FALSE, reverse=FALSE) {
  ...
  nconds <- length(conditions.unique)
  if (model == kModel[12] || nconds < 2 && model %in% kModel[c(5, 6)]) {
    # ignore given conditions for Shapiro-Wilk test or one-sample t-test and
    # Wilcoxon and group into 1 condition instead
    conditions <- rep("", length(vals))
    conditions.unique <- unique(conditions)
  } else if (nconds < 2) {
    # require at least 2 condition to compare, sorting based on reverse arg
  ...
  ci <- NULL
  result <- tryCatchLog::tryCatchLog({
    nconds <- length(val.conds)
    if (model == kModel[5] | model == kModel[7]) {
      # Student's t-test
      if (nconds >= 2) {
        args <- list(val.conds[[2]], val.conds[[1]], paired=paired)
      } else {
        args <- list(val.conds[[1]])
      }
      result <- do.call(t.test, args)
      ...
      eff <- do.call(effectsize::cohens_d, args)
      print(eff)
      ...
      if (nconds >= 2) {
        args <- list(val.conds[[2]], val.conds[[1]], paired=paired, conf.int=TRUE)
      } else {
        args <- list(val.conds[[1]], conf.int=TRUE)
      }
      result <- do.call(wilcox.test, args)
      ...
      effect <- do.call(rcompanion::wilcoxonZ, args) / sqrt(num.per.cond)
      message(
        "Wilcoxon estimate: ", effect.raw, ", standardized effect: ", effect)
    ...
    } else if (model == kModel[12]) {
      # Shapiro-Wilks test of normality
      result <- shapiro.test(val.conds[[1]])
      effect <- result$statistic
...
#'
#' @param stats Data frame generated by \code{\link{statsByRegion}}.
#' @param corr String of correction type to pass to p.adjust.
#' @return Filtered data frame with columns for Region, Effect, and p.
...
#'
#' @param path.in CSV input path for data frame.
#' @param cols Vector of columns to test normality for each column.
#' @return List or test results, named by column.
calcNormality <- function(path.in, cols) {
  # load CSV file
  message("Loading: ", path.in)
  df <- read.csv(path.in)
  
  nors <- list()
  for (col in cols) {
    # test normality
    message("Testing normality of col: ", col)
    vals <- df[[col]]
    vals <- vals[!is.nan(vals)]
    nor <- meansModel(vals, NULL, kModel[12])
    nors[[col]] <- nor
  }
...
#'
#' @param name Name of profile to load. Defaults to NULL, which will initialize  
